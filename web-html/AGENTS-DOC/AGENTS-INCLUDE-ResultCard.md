...
## Notes
- Accepts `$result` associative array with keys: `apellidos`, `nombres`, `nrodni`, `cuit`, `edad`, `sexo`, `provincia`, `ciudad`.
- The name is rendered as `Apellidos, Nombres`. The split happens in PHP with `explode(',', $nombreCompleto)`.
- Sexo: `M` → "Masculino", `F` → "Femenino", otherwise `-`.
- The "Solicitar Informe" button links to `/solicitar?userId=RESULT_ID`.
