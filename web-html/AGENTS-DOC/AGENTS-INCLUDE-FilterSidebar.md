# AGENTS-INCLUDE-FilterSidebar

## Output file
`include/filter-sidebar.php`

## JavaScript needed
None (static markup; sample counts hardcoded for illustration).

## Page content

<div class="row g-2 align-items-end">
  <div class="col-12 col-sm-6 col-md">
    <label class="form-label small fw-semibold mb-1">Sexo</label>
    <select class="form-select form-select-sm">
      <option value="">Todos</option>
      <option value="M">Hombre (15)</option>
      <option value="F">Mujer (12)</option>
    </select>
  </div>
  <div class="col-12 col-sm-6 col-md">
    <label class="form-label small fw-semibold mb-1">Edad</label>
    <select class="form-select form-select-sm">
      <option value="">Todas</option>
      <option value="1">18-30 (8)</option>
      <option value="2">30-50 (12)</option>
      <option value="3">50-100 (7)</option>
    </select>
  </div>
  <div class="col-12 col-sm-6 col-md">
    <label class="form-label small fw-semibold mb-1">Provincia</label>
    <select class="form-select form-select-sm">
      <option value="">Todas</option>
      <option value="bsas">Buenos Aires</option>
      <option value="caba">CABA</option>
      <option value="cordoba">Córdoba</option>
      <option value="santa-fe">Santa Fe</option>
    </select>
  </div>
  <div class="col-12 col-sm-6 col-md">
    <label class="form-label small fw-semibold mb-1">Ciudad</label>
    <select class="form-select form-select-sm">
      <option value="">Todas</option>
      <option value="la-plata">La Plata</option>
      <option value="mar-del-plata">Mar del Plata</option>
      <option value="rosario">Rosario</option>
    </select>
  </div>
</div>
