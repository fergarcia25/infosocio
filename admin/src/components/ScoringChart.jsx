export default function ScoringChart({ score = 0, maxScore = 999 }) {
  const cx = 200, cy = 175, r = 110, sw = 32
  const innerR = r - sw / 2
  ...
  const segments = [
    ...
    { start: 180, end: 252, color: '#EF6C00', label: 'Regular', range: '500 - 649' },
    { start: 252, end: 324, color: '#7CB342', label: 'Bueno', range: '650 - 749' },
  ...
  const statusList = ['BAJO', 'REGULAR', 'REGULAR', 'BUENO', 'EXCELENTE']
    ...
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      <svg viewBox="0 0 400 400" style={{ width: '100%' }}>
        ...
        <g transform={`rotate(${needleAngle}, ${cx}, ${cy})`}>
          <polygon points={`${cx - 7},${cy + 5} ${cx - 7},${cy - 5} ${cx + r - 12},${cy}`} fill="#C62828" />
        </g>
        <circle cx={cx} cy={cy} r={7} fill="#333" />
        <circle cx={cx} cy={cy} r={3} fill="#C62828" />

        {segments.map((s, i) => {
          const mid = (s.start + (s.end <= s.start ? s.end + 360 : s.end)) / 2
          const outer = pt(r + sw / 2 + 3, mid)
          const dist = (mid < 30 || mid > 330) ? r + 18 : (mid > 45 && mid < 135) ? r + 24 : r + 40
          const labelPt = pt(dist, mid)
          const isRight = mid > 300 || mid < 60
        ...
        <text x={cx} y={cy + r + 65} textAnchor="middle" fontSize="28" fill="#333" fontWeight="bold">
          {clamped}
          <tspan fontSize="16" fill="#888"> / {maxScore}</tspan>
        </text>
        <text x={cx} y={cy + r + 87} textAnchor="middle" fontSize="14" fill="#666" fontWeight="bold">
          {status}
        </text>
      </svg>
    </div>
